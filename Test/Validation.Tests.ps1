...
$ModuleRoot = Split-Path (Resolve-Path "$ProjectRoot\PSADHealth\*.psm1")
...
Describe "General project validation: $ModuleName" {
    ...
    If((Test-Path $ProjectRoot\PSADHealth\Config\*.json)){
    It "'$ModuleName' can import cleanly" {
        {Import-Module (Join-Path $ModuleRoot "PSADHealth.psd1") -Force} | Should -Not -Throw
    }
    }
}